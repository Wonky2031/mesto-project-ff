...
import { getAllCardsDataRequest, getCardDataRequest, getCardLikesCount, getUserDataRequest, deleteCardLikeRequest, addCardLikeRequest, deleteCardDataRequest, updateAvatarRequest, addCardDataRequest, updateProfileInformation } from './api';
import './index.css';
...
const editProfileImagePopup = document.querySelector('.popup_type_edit-profile-image');
const editProfileImageForm = document.forms['edit-profile-image'];
const newAvatarUrlInput = editProfileImageForm.querySelector('.popup__input_type_url');
...
const inputPatternError = 'Разрешены только латинские, кириллические буквы, знаки дефиса и пробелы';
...
function submitNewCardForm(evt) {
  ...
  if (isValid) {
    ...
    Promise.all([getUserDataRequest(), addCardDataRequest(cardNameInput.value, cardImageUrl.value)])
...
function submitNewAvatarForm(evt) {
  evt.preventDefault();
  const inputs = editProfileImageForm.querySelectorAll(validationSettings.inputSelector);
  let isValid = true;
  inputs.forEach(input => {
    if (!input.validity.valid) {
      isValid = false;
...
function updateCardLikes(likeButton, cardId, likeFunction){
  const likesCount = likeButton.parentElement.querySelector('.card__like-count');

  Promise.all([getUserDataRequest(), getCardDataRequest(cardId)])
  .then(([userData, cardData]) => {
    const likedUsersJson = cardData.likes;
    const isAlreadyLiked = likedUsersJson.some(user => user._id === userData._id)
    if(isAlreadyLiked){
